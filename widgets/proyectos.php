...
class Custom_Proyectos extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'sydney_portfolio_widget', 'description' => __( 'Muestra los proyectos del grupo.', 'sydney'), 'panels_groups' => array('GISAI'), 'panels_icon' => 'dashicons dashicons-portfolio'  );
       parent::__construct(false, $name = __('Página Principal: Proyectos', 'sydney'), $widget_ops);
		$this->alt_option_name = 'custom_proyectos';
  }
  ...
  public function widget( $args, $instance ) {
    ...
    $output = ''; //Start output
    $output .= '<div class="project-wrap portfolio-section ' . $style . '">';
    ...
    while ( $the_query->have_posts() ):
       ...
       if ( $termsArray) {
           foreach ( $termsArray as $term ) {
			 ...
			 $project_title .= '<h3 class="project-title">';
       ...
       if ( has_post_thumbnail() ) {
           ...
           if ( $project_url ) :
               $output .= '<div class="project-item item isotope-item ' . $termsString . '">';
							 $output .= '<div class="project-inner">';
