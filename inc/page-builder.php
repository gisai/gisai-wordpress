...
add_theme_support( 'siteorigin-panels', array( 
	'margin-bottom' => 0,
	'post-types'	=> array('post', 'page', 'projects', 'employees')
) );
...
function sydney_theme_widgets($widgets) {
	$theme_widgets = array(
		...
		'Sydney_Employees',
		'Sydney_Employees_Type_B',
		'Sydney_Timeline',
		'Sydney_Latest_News',
		'Sydney_FP_Contact_Info',
		'Sydney_Latest_News_Type_B',
		'Sydney_Portfolio',
		'Sydney_Image_Widget',
		'Sydney_Pricing_Tables'
	);
...
function sydney_row_styles_output($attr, $style) {
	...
	if(!empty( $style['background_image'] )) {
		...
		if( !empty($url) ) {
			...
			$attr['data-siteorigin-parallax'] = json_encode( $parallax_args );
		}
	}
...
function sydney_custom_widget_style_fields($fields) {
	...
	$fields['title_alignment'] = array(
	    'name'        => __('Widget title alignment', 'sydney'),
		'type' 		  => 'select',
	    'group'       => 'design',
		'options' => array(
			'left' => __('Left', 'sydney'),
